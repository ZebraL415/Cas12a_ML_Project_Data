...
- Table S3 原始 `guide_target_hamming_dist` 与保留 `-` 的 25 位字符对齐直接计数一致率为 98.232%，但仍有 188 行、28 个唯一 pair 不一致；该列很可能主要表示 aligned distance，但异常行仍需逐条核验，不能作为默认训练特征。
...
- Table S3 有 740 条 `target_at_guide` 含 `-`。需要结合论文模型输入代码确认每行 gap 的事件方向，再生成 gap-aware mismatch 特征；在此之前不得使用删除 gap 后的 Xu 位置特征。
- 林同学 Table S2 扫描中的 642 个可回连唯一 pair 存在多模板映射；需要保存全部并列命中并定义 `mapping_status`，不能把最早命中当作唯一来源。
- 项目 standalone `Table S2.xlsx` 实际包含 Training/Augment/Test，`Table S3.xlsx` 实际包含四种病原体 DNA/crRNA；原文件不修改，但 catalog 需要按实际内容纠正说明。
