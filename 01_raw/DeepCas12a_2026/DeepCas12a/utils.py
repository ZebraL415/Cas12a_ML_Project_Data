...
class Episgt:
    def __init__(self, fpath, num_epi_features, with_y=True):
        self._fpath = fpath
        self._ori_df = pd.read_csv(fpath, sep=r"\s+", header=None)

        self._num_epi_features = num_epi_features
        self._with_y = with_y
        self._num_cols = num_epi_features + 2 if with_y else num_epi_features + 1

        # Keep only the sequence, epigenetic feature, and optional label columns.
        self._cols = list(self._ori_df.columns)[-self._num_cols:]

        # Build the working dataframe used by the encoder.
